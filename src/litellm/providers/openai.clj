...
(defn transform-messages
  ...
  Echoes an assistant message's `:tool-calls` back to the provider — OpenAI requires
  the assistant message that issued the `tool_calls` to precede the corresponding
  `:tool` result messages, so dropping it breaks any multi-turn tool conversation.
  Assistant tool-call messages legitimately carry nil `:content`."
  [messages]
  (map (fn [msg]
